...
執行：
```bash
uv run python src/fine_tuning/generate_data.py
```
這會在 `src/fine_tuning` 目錄下生成 `data.jsonl`。
...
執行：
```bash
uv run python src/fine_tuning/fine_tuning.py
```
* 注意：第一次執行會需要下載模型，請耐心等候。
* 訓練完成後，Adapters 會儲存在 `src/fine_tuning/qwen-1.5b-local-adapter` 目錄。
...
執行：
```bash
uv run python src/fine_tuning/merge_model.py
```
此腳本會：
1. 將 LoRA Layers 合併到 Base Model。
2. 將完整模型儲存至 `src/fine_tuning/merged_model`。
3. 自動產生 `src/fine_tuning/Modelfile`。
